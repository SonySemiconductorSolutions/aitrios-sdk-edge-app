...
extern void memory_usage(size_t used, size_t fordblks, size_t keepcost);
void *malloc(size_t size) {
  void *result = dlmalloc(size);
  struct mallinfo info = dlmallinfo();
  memory_usage(info.uordblks, info.fordblks, info.keepcost);
  return result;
}
...
void *calloc(size_t nmemb, size_t size) {
  void *result = dlcalloc(nmemb, size);
  struct mallinfo info = dlmallinfo();
  memory_usage(info.uordblks, info.fordblks, info.keepcost);
  return result;
}
