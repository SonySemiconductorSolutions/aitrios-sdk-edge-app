...
/*
  ...
  thus be inaccurate.
*/
extern __attribute__((visibility("default"))) struct mallinfo dlmallinfo(void);
#endif /* NO_MALLINFO */
...
*/
extern __attribute__((visibility("default"))) void dlmalloc_stats(void);
...
#if defined(__GNUC__) && \
    (defined(__i386__) || defined(__x86_64__) || defined(__wasm__))
...
#ifdef __wasilibc_unmodified_upstream  // Bug fix: set ENOMEM on footprint
                                       // overrun
    if (fp <= m->footprint || fp > m->footprint_limit) return 0;
#else
...
#if __wasilibc_unmodified_upstream  // Forward declaration of
                                    // try_init_allocator.
#else
...
#ifdef __wasilibc_unmodified_upstream  // Define a function that initializes the
                                       // initial state of dlmalloc.
#else
...
/* Initialize the initial state of dlmalloc to be able to use free memory
 * between __heap_base and initial. */
static void try_init_allocator(void) {
  ...
  if (end < base) {
    ...
    // reference.
    //
    // Note: This is a linker bug:
    // https://github.com/llvm/llvm-project/issues/60829
    __builtin_trap();
  }
  size_t initial_heap_size = end - base;

  /* Check that initial heap is long enough to serve a minimal allocation
   * request. */
