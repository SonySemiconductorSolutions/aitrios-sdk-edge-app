...
class DataProcessorUtilsTest : public ::testing::TestWithParam<int> {
 protected:
  void SetUp() override {
...
TEST_F(DataProcessorUtilsTest, GetValueNumberTest) {
...
TEST_P(DataProcessorUtilsTest, SetEdgeAppLibNetwork) {
...
INSTANTIATE_TEST_CASE_P(DataProcessorUtilsTestP, DataProcessorUtilsTest,
                        ::testing::Range(0, 100));

TEST_F(DataProcessorUtilsTest, SetEdgeAppLibNetworkErrorNotAvailable) {
  char buf[256];
  ...
  JSON_Object *object = json_object(value);
  // Call SetEdgeAppLibNetwork and check for leaks
  int res = SetEdgeAppLibNetwork(0, object);
  // Free the JSON_Value to avoid memory leaks
  json_value_free(value);
...
TEST_F(DataProcessorUtilsTest, SetEdgeAppLibNetworkErrorTooLong) {
  char buf[256];
  snprintf(buf, sizeof(buf), "{\"ai_model_bundle_id\":\"%s\"}",
