...
EdgeAppLibSensorImageCropProperty roi[2] = {
    {0, 0, 2016, 1520},
    {0, 0, detection_param.input_width,
     detection_param
         .input_height}  // It must be smaller than input tensor size of imx500
...
int onIterate() {
  ...
  for (int i = 1; i < model_count; ++i) {
    ...
    LOG_DBG(
        "for roi[%d]: [left=%d, top=%d, width=%d, "
        "height=%d]",
        i, roi[i].left, roi[i].top, roi[i].width, roi[i].height);

    frame = EdgeAppCore::Process(*ctx_list[i], shared_list[i], frame)
                .withROI(roi[i])
                .withPreprocessing(PreprocessLib::grayscalePreprocess)
                .compute();
    ...
    auto cpu_outputs = EdgeAppCore::GetOutputs(*ctx_list[i], frame, 1);
    LOG_INFO("Got %zu output tensors from CPU model", cpu_outputs.size());
    if (cpu_outputs.empty()) {
      LOG_ERR("No output tensors from CPU model.");
      return -1;
    }
    // Analyze (Read Plate)
    char *recognized_data = NULL;
    uint32_t recognized_data_size = 0;
    LPRDataProcessorAnalyze(cpu_outputs, &recognized_data,
                            &recognized_data_size);

    // Send the result even if it's not a valid Japanese number plate
    if (recognized_data != NULL) {
