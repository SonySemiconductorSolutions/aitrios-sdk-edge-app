...
static const std::vector<std::string> LP_CATEGORIES_OCR = {
    "?",  "上", "越", "5",  "4",  "き", "6",  "7",  "-",  "2",  "8",  "船",
    "橋", "あ", "3",  "1",  "川", "0",  "て", "長", "岡", "か", "姫", "路",
    "れ", "9",  "江", "東", "う", "豊", "田", "ひ", "品", "く", "春", "日",
    "井", "ま", "野", "え", "袖", "ヶ", "浦", "い", "・", " ",  "高", "崎",
    "そ", "帯", "広", "知", "床", "な", "ね", "釧", "ち", "習", "志", "の",
    "熊", "谷", "と", "市", "は", "板", "葛", "飾", "苫", "小", "牧", "た",
    "成", "伊", "豆", "足", "立", "新", "潟", "旭", "せ", "富", "山", "さ",
    "所", "沢", "つ", "鈴", "鹿", "む", "多", "摩", "堺", "り", "八", "戸",
    "金", "和", "泉", "沼", "津", "大", "阪", "神", "室", "蘭", "尾", "張",
    "ふ", "一", "宮", "ほ", "飛", "鳥", "杉", "並", "を", "岩", "手", "松",
    "本", "け", "京", "都", "士", "弘", "前", "ぬ", "三", "重", "み", "湘",
    "南", "こ", "勢", "歌", "青", "森", "に", "部", "北", "見", "石", "す",
    "原", "わ", "口", "横", "浜", "函", "館", "滋", "賀", "諏", "訪", "柏",
    "名", "古", "屋", "静", "奈", "良", "世", "練", "馬", "札", "幌", "四",
    "め", "千", "葉", "相", "模", "福", "王", "子", "梨", "河", "城", "岐",
    "阜", "那", "須", "群", "ぎ", "騨", "仙", "台", "形", "取", "徳", "島",
    "佐", "ゆ", "分", "ろ", "下", "関", "九", "州", "よ", "久", "留", "米",
    "も", "香", "筑", "や", "根", "る", "愛", "媛", "出", "雲", "沖", "縄",
    "倉", "敷", "児", "保", "ら", "奄", "美", "会", "盛", "水", "秋", "平",
    "白", "庄", "内", "郡", "ば", "土", "宇", "F",  "Y",  "X",  "A",  "B",
    "C",  "L",  "H",  "M",  "P",  "I",  "K",  "E"};
...
// Function to expose LP_CATEGORIES_OCR for external use
const std::vector<std::string> &GetLPCategoriesOCR() {
  return LP_CATEGORIES_OCR;
}
...
Detections *CreateLPDetectionsFromTensors(
    const std::vector<EdgeAppCore::Tensor> &tensors,
    DataProcessorCustomParam_LPD detection_param) {
  if (tensors.size() < 4) {
    LOG_ERR("Expected at least 4 tensors, got %zu", tensors.size());
    return nullptr;
  }

  // Extract tensor data
  float *scores = static_cast<float *>(tensors[0].data);
  float *coords = static_cast<float *>(tensors[1].data);
  float *valid_count = static_cast<float *>(tensors[2].data);
  float *class_ids = static_cast<float *>(tensors[3].data);

  int num_detections = static_cast<int>(valid_count[0]);
  LOG_DBG("Number of detections from valid_count: %d", num_detections);

  Detections *detections =
      static_cast<Detections *>(malloc(sizeof(Detections)));
  if (!detections) {
    LOG_ERR("Failed to allocate memory for detections.");
    return nullptr;
  }
  ...
  detections->detection_data = static_cast<DetectionData *>(
      malloc(sizeof(DetectionData) * num_detections));
  if (!detections->detection_data && num_detections > 0) {
    LOG_ERR("Failed to allocate memory for detection data.");
    free(detections);
    return nullptr;
  }

  int total_tensors = tensors[1].shape_info.dims[0];  // IMX500 specific
  LOG_DBG("Total tensors: %d", total_tensors);
  // Process each detection
  for (int i = 0; i < num_detections && i < detection_param.max_detections;
       ++i) {
    DetectionData &det = detections->detection_data[i];
    det.score = scores[i];
    det.class_id = static_cast<int>(class_ids[i]);

    if (detection_param.bbox_normalized) {
      det.bbox.top =
          coords[i + total_tensors * ymin_index] * detection_param.input_height;
      det.bbox.left =
          coords[i + total_tensors * xmin_index] * detection_param.input_width;
      det.bbox.bottom =
          coords[i + total_tensors * ymax_index] * detection_param.input_height;
      det.bbox.right =
          coords[i + total_tensors * xmax_index] * detection_param.input_width;
    } else {
      det.bbox.top = coords[i + total_tensors * ymin_index];
      det.bbox.left = coords[i + total_tensors * xmin_index];
      det.bbox.bottom = coords[i + total_tensors * ymax_index];
      det.bbox.right = coords[i + total_tensors * xmax_index];
    }
  }
