...
struct CountData FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  typedef CountDataBuilder Builder;
  enum FlatBuffersVTableOffset FLATBUFFERS_VTABLE_UNDERLYING_TYPE {
    VT_CLASS_ID = 4,
    VT_COUNT = 6
  };
...
struct CountDataBuilder {
  typedef CountData Table;
  ::flatbuffers::FlatBufferBuilder &fbb_;
  ::flatbuffers::uoffset_t start_;
  void add_class_id(uint32_t class_id) {
    fbb_.AddElement<uint32_t>(CountData::VT_CLASS_ID, class_id, 0);
  }
  void add_count(uint32_t count) {
    fbb_.AddElement<uint32_t>(CountData::VT_COUNT, count, 0);
  }
  explicit CountDataBuilder(::flatbuffers::FlatBufferBuilder &_fbb)
        : fbb_(_fbb) {
    start_ = fbb_.StartTable();
  }
  ::flatbuffers::Offset<CountData> Finish() {
    const auto end = fbb_.EndTable(start_);
    auto o = ::flatbuffers::Offset<CountData>(end);
    return o;
  }
};

inline ::flatbuffers::Offset<CountData> CreateCountData(
    ::flatbuffers::FlatBufferBuilder &_fbb,
    uint32_t class_id = 0,
    uint32_t count = 0) {
  CountDataBuilder builder_(_fbb);
